...
describe('integration', () => {
  it('applies everything at once', async () => {
    ...
    assert.equal(config.get('remote.title'), 'Essence Mascara Lash Princess')
    assert.equal(config.get('remote.price'), 9.99)
    assert.equal(config.get('remote.formatted'), 'Essence Mascara Lash Princess - 9.99')
